...
\item
  ...
  \href{https://en.wikipedia.org/wiki/Pulse-code_modulation}{PCM
    signal} and a loss of information. In the general case,
  quantization maps the real (analog) value of samples into a discrete
  set~\cite{vetterli1995wavelets}.
...
\item
  ...
  levels). $\{r_i\}$ must be finite. % The set $\{r_i\}$ is called the
                                     % codebook and the $r_i$ the
                                     % codewords.
...
\item The performance of the quantizer is measured as the distance
  ...
  Thus, the MSE is
  \begin{equation}
    D = E(|x-y|^2)=\sum_i\int_{x_{i-1}}^{x_i} (x-y_i)^2f_X(x)dx,
  \end{equation}
